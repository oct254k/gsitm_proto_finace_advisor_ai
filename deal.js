...
(function(){
  ...
  function riskTable(){
    const rows=[
      ['warn','매출 집중','한전 단일 매출 의존도 64.3%','한전 발주 정책 변화 시 매출 급감. <span style="color:var(--red);font-weight:600">한일변전 유사 리스크 현실화 → IRR -3.3%p</span>',94,'high','상','[리스크] 매출집중. 한전 의존도 64.3%. 한일변전 IRR -3.3%p 사례. 신뢰도 94%, 심각도 상.'],
    ...
    return `<div class="risk-table"><div class="risk-head"><div style="text-align:center">유형</div><div>내용 · 근거</div><div>신뢰도</div><div style="text-align:center">심각도</div></div>${rows.map(r=>`<div class="risk-row sendable" data-ctx="${r[7]}" data-label="리스크: ${r[1]}"><div style="text-align:center"><span class="badge ${r[0]}" style="height:21px">${r[1]}</span></div><div><div class="risk-main">${r[2]}</div><div class="risk-detail">${r[3]}</div></div><div><div class="conf-mini"><div class="conf-bar-mini"><i style="width:${r[4]}%"></i></div>${r[4]}%</div></div><div style="text-align:center"><span class="sev-${r[5]}">${r[6]}</span></div><button class="send-to-ai">AI 분석 →</button></div>`).join('')}</div>`;
  }
